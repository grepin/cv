...
from tests.api.functional.testdata.films import ONE_FILM, ONE_FILM_WITH_STANDARD_SUBSCRIPTION
from tests.api.functional.utils import get_cache_key
...
@pytest.mark.parametrize("film", [ONE_FILM_WITH_STANDARD_SUBSCRIPTION])
async def test_get_one_film_with_subscription_no_auth(
    film,
    api_v1_path,
    make_get_request,
    redis_flushall
    ...
    assert response.body == {'detail': 'subscription needed'}
