...
from tests.api.functional.testdata.persons import EXISTING_PERSONS
...
async def test_api_v1_person_films(api_v1_path, make_get_request, redis_flushall):
    ...
    for _ in ["not cached", "cached"]:
        for person in EXISTING_PERSONS:
            ...
            assert response.status == HTTPStatus.OK
            print("FILM", person["film_ids"])
            films = [
                {k: v for k, v in film.items() if k in ["uuid", "title", "imdb_rating", "subscription"]}
                for film in person["film_ids"]
            ]
            print("FILMS", sorted(films))
            print("BODY`", sorted(response.body))
